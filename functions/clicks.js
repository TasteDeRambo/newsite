...
exports.handler = async (event, context) => {
  ...
  const client = new faunadb.Client({ secret: process.env.fnAFXlEltRAASacUr_LudawtkoweIlm11bbAiOfD });
  ...
  if (!doesDocExist) {
    await client.query(q.Create(q.Collection('clicks'), { data: { slug: 'clickCount', clicks: 1 } }));
  }
